  ...
  return (
    <>
      <Button
        variant="ghost"
        size="icon"
        className={`fixed top-3 sm:top-4 z-50 bg-background rounded-lg h-9 w-9 sm:h-10 sm:w-10 ${
          offsetLeft ? "left-12 sm:left-14" : "left-3 sm:left-4"
        }`}
          ...
          <nav className="space-y-0.5 sm:space-y-1">
            {filteredNavItems.map((item) => (
              ...
              >
                <Button
                  variant="ghost"
                  className="w-full justify-start gap-2 sm:gap-3 text-sm sm:text-base h-10 sm:h-12 px-3 sm:px-4"
                >
                  <item.icon className="h-4 w-4 sm:h-5 sm:w-5" />
                  {item.label}
                  {item.href === "/powiadomienia" &&
