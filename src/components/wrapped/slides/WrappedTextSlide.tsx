...
export function WrappedTextSlide({ lines, accentColor = 'rgba(99, 102, 241, 0.3)' }: WrappedTextSlideProps) {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center px-8 text-white text-center relative overflow-hidden bg-[#1a1a2e]">
      ...
      <div className={`relative z-10 max-w-5xl space-y-2 transition-all duration-500 ${
        isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
      }`}>
        {lines.map((line, i) => (
          <p
            key={i}
            className="text-5xl md:text-6xl lg:text-8xl font-black tracking-tight leading-tight"
          >
            {line}
          </p>
        ))}
